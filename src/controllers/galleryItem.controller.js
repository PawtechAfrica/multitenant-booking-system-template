...
const service = require('../services/galleryItem.service')

const listPublic = asyncHandler(async (req, res) => {
  const data = await service.listPublic(req.property.id, req.query.category)
  res.status(200).json({ success: true, data })
})

const listAdmin = asyncHandler(async (req, res) => {
  const data = await service.listAdmin(req.property.id, req.query.category)
  res.status(200).json({ success: true, data })
})
...
const remove = asyncHandler(async (req, res) => {
  ...
  res.status(200).json({ success: true, data: { message: 'Gallery item deleted.' } })
})
